...
#define MAX_ACCEPTED_PIXEL_ERROR    0.01 // максимальное расстояние в пикселях (процент от ширины картинки) между ключевыми точками чтобы их можно было зачесть как "почти совпавшие" (это очень завышенный порог, по-хорошему должно быть 0.005 например)
...
template <typename SIFT>
MatchingPairData evaluateMatching(SIFT &sift, const cv::Mat &img1, const cv::Mat &img2, const std::string &output_matches_img_path)
{
...

    {
        for (int method = 0; method < 3; ++method) { // тестируем три метода: OpenCV ORB, OpenCV SIFT, ваш SIFT
            ...
            if (method == 0) {
                ...
                method_name = "SIFTOCV";
                log_prefix = "[SIFTOCV] ";
                cv::Ptr<cv::FeatureDetector> detector = cv::SIFT::create(p.nfeatures, p.n_octave_layers, p.contrast_threshold, p.edge_threshold); // здесь можно было бы поиграть с его параметрами, например выделять больше чем 500 точек, строить большее число ступеней пирамиды и т.п.
                detector->detect(img0, kps0); // детектируем ключевые точки на исходной картинке
                detector->detect(img1, kps1); // детектируем ключевые точки на преобразованной картинке
...
// ── Octave serialization ──────────────────────────────────────────────────
    ...
    void saveOctaves(const std::string& path, const std::vector<phg::SIFT::Octave>& octaves) {
        ...
        if (DENY_CREATE_REF_DATA) throw std::runtime_error("saving reference data is denied");
...
// If descriptor rows are provided (non-null), they must also be similar.
    bool keypointsSimilar(const cv::KeyPoint& a, const cv::KeyPoint& b,
                          double relEps,
                          const double* descRowA = nullptr,
                          const double* descRowB = nullptr,
                          int descCols = 0)
    {
        ...
        if (!relClose(a.pt.x, b.pt.x))       return false;
        if (!relClose(a.pt.y, b.pt.y))       return false;
        if (!relClose(a.size, b.size))         return false;
        if (!relClose(a.response, b.response)) return false;
        // For angle: use absolute threshold (relEps * 360) to handle near-zero angles
        if (angleDist(a.angle, b.angle) > relEps * 360.0) return false;
        // If descriptors are provided, they must also match
        if (descRowA && descRowB && descCols > 0 && !descriptorRowsSimilar(descRowA, descRowB, descCols, 4 * relEps)) return false;
        return true;
    }
...
//   2) at least 80% of detected keypoints have a similar reference keypoint
    void compareKeypoints(const std::vector<cv::KeyPoint>& ref,
                          const std::vector<cv::KeyPoint>& detected,
                          const std::string& stepName,
                          const cv::Mat& refDesc = cv::Mat(),
                          const cv::Mat& detDesc = cv::Mat()) {
        ...
        EXPECT_LE(countRatio, kCountTolerance)
                            << stepName << ": keypoint count out of tolerance — detected "
                            << detected.size() << ", reference " << ref.size()
                            << " (diff " << (countRatio * 100.0) << "%)";
        ...
        EXPECT_GE(matchRate, kMinMatchRate)
                            << stepName << ": only " << goodCount << " / " << detected.size()
                            << " detected keypoints (" << (matchRate * 100.0)
                            << "%) matched a reference keypoint (need "
                            << (kMinMatchRate * 100.0) << "%)"
                            << (useDesc ? " [with descriptors]" : "");

        std::cout << "[  MATCH  ] " << stepName << ": " << goodCount << "/"
                  << detected.size() << " matched ("
                  << (matchRate * 100.0) << "%), ref count=" << ref.size()
                  << (useDesc ? " [with descriptors]" : "")
                  << std::endl;
    }
...
// ── Generic "check or create" wrappers ────────────────────────────────────
    ...
    void checkOrCreateOctaves(const std::string& filename,
                              const std::vector<phg::SIFT::Octave>& octaves,
                              const std::string& stepName) {
        std::string path = kDataDir + filename;
        if (fs::exists(path)) {
            auto ref = loadOctaves(path);
            compareOctaves(ref, octaves, stepName);
            std::cout << "[  CHECK  ] " << stepName
                      << ": matched reference " << filename << std::endl;
        } else {
            saveOctaves(path, octaves);
            std::cout << "[  CREATE ] " << stepName
                      << ": saved reference " << filename << std::endl;
        }
    }
...
// saved/loaded alongside keypoints and included in the similarity check.
    void checkOrCreateKeypoints(const std::string& filename,
                                const std::vector<cv::KeyPoint>& kpts,
                                const std::string& stepName,
                                const std::string& descFilename = "",
                                const cv::Mat& desc = cv::Mat()) {
        std::string path = kDataDir + filename;
        ...
        if (fs::exists(path)) {
            ...
            std::cout << "[  CREATE ] " << stepName
                      << ": saved reference " << filename
                      << (withDesc ? " + " + descFilename : "")
                      << std::endl;
        }
    }
